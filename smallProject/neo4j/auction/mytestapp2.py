...
class AuctionApp:
    ...
    def list_auctions(self):
        with self._driver.session() as session:
            ...
            return [record["c"].values for record in result]

    def place_bid(self, user_id, car_id, amount):
        with self._driver.session() as session:
            session.run(
                "MATCH (user:User {id: $user_id}), (car:Car {id: $car_id}) "
                "CREATE (user)-[:PLACED]->(bid:Bid {amount: $amount})-[:ON]->(car)",
                user_id=user_id,
                car_id=car_id,
                amount=amount,
            )
    ...
    def highest_bid(self, car_make):
        with self._driver.session() as session:
            result = session.run(
                "MATCH (:User)-[:PLACED]->(bid:Bid)-[:ON]->(car:Car {make: $car_make}) "
    ...
    def highest_bidder(self, car_make):
        with self._driver.session() as session:
            result = session.run(
                "MATCH (user:User)-[:PLACED]->(bid:Bid)-[:ON]->(car:Car {make: $car_make}) "
